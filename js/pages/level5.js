...
export function renderLevel5() {
    ...
    const sections = [
        ...
        textPanel("Введение", [
            p("Внутренняя телеметрия PostgreSQL - это встроенные средства сбора и представления статистики о внутренних процессах СУБД и низкоуровневых операциях (WAL, фоновый писатель, кеш-блоки, память и т.д.).",
        ...
        textPanel("pg_stat_io", [
            ...
            ul([
                ...
                "hits - число попаданий в кеш",
                "evictions - число вытеснений из кеша",
        ...
        textPanel("Ключевые метрики внутренней телеметрии", [
            ...
            p("Буферный кеш:", { strong: ["буферный кеш"] }),
            ul([
                "число попаданий в кеш (hits) из pg_stat_io",
                "число вытеснений из кеша (evictions)"
            ]),
            p("Высокое hits - эффективное использование памяти, большой evictions - недостаток кеш-памяти (нужно увеличить shared_buffers).",
            { code: ["shared_buffers"] }),
        ...
        quizPanel({
            ...
            explanation: "В pg_stat_io используются следующие контексты: normal - стандартные операции через буферный кеш, init - операции при инициализации WAL сегментов, vacuum - I/O при вакуумировании, bulkread и bulkwrite - большие пакетные операции чтения/записи (например, COPY)."
        }),
        ...
        quizPanel({
            ...
            explanation: "Колонка evictions показывает, сколько раз блок был вытеснен из кеша из-за нехватки места. Высокий показатель указывает на то, что кеш памяти переполняется, и следует увеличить параметр shared_buffers."
        }),
